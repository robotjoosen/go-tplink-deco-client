...
// GenerateRsaKey creates an RSA Public key from an exponent and modulus
func GenerateRsaKey(modulus string, exponent string) (*rsa.PublicKey, error) {
	n := new(big.Int)
	n.SetString(modulus, 16)

	e, err := strconv.ParseInt(exponent, 16, 0)
	if err != nil {
		return nil, err
	}
